...
use App\Filament\Exports\AbstractSubmissionExporter;
...
class AbstractSubmissionExcelExportService
{
    private const HYPERLINK_COLUMNS = [
        'abstract_file' => 'Xem abstract',
        'cv_file' => 'Xem CV',
        'headshot_file' => 'Xem headshot',
        'degree_file' => 'Xem degree',
    ];
    ...
    {
        $query = AbstractSubmissionExporter::modifyQuery($query);
        $fileName = 'abstract-sirhcm2026-'.now()->format('Y-m-d-His').'.xlsx';
        $columnKeys = array_keys($columnMap);

        $export = new Export([
            'exporter' => AbstractSubmissionExporter::class,
        ]);

        $exporter = new AbstractSubmissionExporter($export, $columnMap, []);
